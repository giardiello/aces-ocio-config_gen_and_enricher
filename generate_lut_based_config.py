...
def _fix_clf_formatting(path):
    """Fix XML declaration, Array indentation, and Range value whitespace.

    Addresses three issues that ET.indent() cannot handle:
    1. Single-quoted XML declaration → double-quoted
    2. <Array> data lines not indented to match their nesting depth
    3. <Range> value elements with padded text like <minInValue> 0 </minInValue>
    """
    import re
...
def postprocess_clf(clf_path, name, clf_id, input_desc, output_desc, op_descriptions):
    ...
    tree.write(clf_path, xml_declaration=False, encoding='UTF-8')
    _fix_clf_formatting(clf_path)
...
def probe_acescct_display_range(config, vt_name, grid_res=65, margin=0.01):
    """
...
def _make_domain_normalize(domain_min, domain_max):
    """RangeTransform that maps [domain_min, domain_max] → [0, 1] with clamp.

    Serialises as CLF <Range> (clamped) — the proper CLF element for domain
    remapping.  Clamping is desirable: it guarantees the downstream LUT3D
    receives input strictly within [0, 1].
    """
    r = ocio.RangeTransform()
    r.setMinInValue(domain_min)
    r.setMaxInValue(domain_max)
...
def _make_domain_denormalize(domain_min, domain_max):
    """RangeTransform that maps [0, 1] → [domain_min, domain_max] with clamp.

    Serialises as CLF <Range> (clamped) — the proper CLF element for domain
    remapping.  Clamping is desirable: it ensures the upstream LUT3D output
    stays within the probed domain before entering the analytical decode.
    """
    r = ocio.RangeTransform()
    r.setMinInValue(0.0)
    r.setMaxInValue(1.0)
    r.setMinOutValue(domain_min)
    r.setMaxOutValue(domain_max)
    return r
...
#   HDR views  -> per-VT gamma (tuned to peak nits) at AP1 primaries
#
...
# Analytical shaper gamma for HDR views, derived by matching PQ's perceptual
# distribution over the restricted [0, peak_nits/10000] range.
# Linear fit: gamma = 1.0514 + 0.2403 * log2(peak_nits)
_HDR_GAMMA_INTERCEPT = 1.0514
_HDR_GAMMA_SLOPE = 0.2403


def _extract_peak_nits(vt_name):
    """Extract peak luminance in nits from a VT name, or 0 if not found."""
    import re as _re
    m = _re.search(r'(\d+)\s*nit', vt_name)
...
def _is_hdr_view(vt_name):
    ...
    if 'SDR' in vt_name:
        return False
    if 'HDR' in vt_name:
        return _extract_peak_nits(vt_name) > 108
    return False


def _hdr_shaper_gamma(vt_name):
    """Compute the optimal analytical shaper gamma for an HDR VT.

    The gamma is tuned per peak luminance to match PQ's perceptual
    distribution over the VT's actual output range.  Higher peak
    luminance requires a higher gamma (more shadow expansion) because
    the ratio between peak and shadow luminance is larger.
    """
    nits = _extract_peak_nits(vt_name)
    if nits <= 0:
        nits = 1000
    return _HDR_GAMMA_INTERCEPT + _HDR_GAMMA_SLOPE * math.log2(nits)


def _get_display_shaper_label(vt_name):
    """Return a human-readable label for the shaper used by this VT."""
    if _is_hdr_view(vt_name):
        gamma = _hdr_shaper_gamma(vt_name)
        return f"Gamma {gamma:.2f} (tuned for {_extract_peak_nits(vt_name)} nits)"
    return "Gamma 2.2"


def _gamma_encode_op(gamma):
    """Linear AP1 -> gamma-encoded AP1: x^(1/gamma), negatives clamped to 0."""
    return ocio.ExponentTransform(
...
def _gamma_decode_op(gamma):
    """Gamma-encoded AP1 -> Linear AP1: x^gamma, negatives clamped to 0."""
    return ocio.ExponentTransform(
        value=[gamma, gamma, gamma, 1.0],
        negativeStyle=ocio.NEGATIVE_CLAMP,
        direction=ocio.TRANSFORM_DIR_FORWARD,
...
def get_gamma22_encode_op():
    ...
    return _gamma_encode_op(GAMMA22_EXPONENT)


def get_gamma22_decode_op():
    """Gamma 2.2 encoded AP1 -> Linear AP1: x^2.2, negatives clamped to 0."""
    return _gamma_decode_op(GAMMA22_EXPONENT)


def _get_shaper_encode_decode(vt_name):
    """Return (encode_op, decode_op) for the appropriate display shaper.

    SDR views use a fixed gamma 2.2.  HDR views use a per-VT gamma tuned
    to the peak luminance — higher peaks get higher gamma for better shadow
    precision, matching PQ's perceptual distribution analytically.
    """
    if _is_hdr_view(vt_name):
        gamma = _hdr_shaper_gamma(vt_name)
        return _gamma_encode_op(gamma), _gamma_decode_op(gamma)
    return get_gamma22_encode_op(), get_gamma22_decode_op()


def probe_display_shaper_range(config, vt_name, grid_res=65, margin=0.01):
    """
    ...
    Sweeps a dense 3D grid of ACEScct(scene) values through:
      ACEScct decode → AP0 → VT fwd → XYZ→AP1 → display encode (gamma)
...
def _get_shaper_clf_ops(vt_name):
    ...
    Both SDR and HDR use analytical ExponentTransform (gamma power curve),
    which serialises directly as CLF <Exponent style="basicFwd/Rev">.
    HDR views use a per-VT gamma tuned to the peak luminance.
    """
    if _is_hdr_view(vt_name):
        gamma = _hdr_shaper_gamma(vt_name)
        return [_gamma_encode_op(gamma)], [_gamma_decode_op(gamma)]
...
def build_forward_clf_display_shaper(config, cube_path, clf_path, vt_name,
    ...
    Build forward CLF (display-shaper domain):
      ...
      [+ denormalize] + display decode (gamma) + AP1->XYZ-D65.
    Input: ACES2065-1.  Output: CIE-XYZ-D65.
    """
    untm_fwd_ops, _ = get_untm_ops(config)
    ap0_to_ap1, log_enc = get_acescct_encode_ops()
...
def build_inverse_clf_display_shaper(config, cube_path, clf_path, vt_name,
    ...
    Build inverse CLF (display-shaper domain):
      XYZ-D65->AP1 + display encode (gamma) [+ normalize]
...
def _fwd_op_descriptions_dispshaper(has_domain_range):
    descs = [
        "ACES AP0 to AP1 matrix",
        "Linear to ACEScct log (encode for LUT input)",
        "3D LUT - ACEScct(scene) to display-encoded AP1",
    ]
    if has_domain_range:
        descs.append("Denormalize [0,1] to display-shaper domain")
    descs += [
        "Display decode (gamma) to linear AP1",
        "ACES AP1 to AP0 matrix",
        "AP0 to CIE-XYZ-D65 (Un-tone-mapped forward)",
    ]
    return descs
...
def _inv_op_descriptions_dispshaper(has_domain_range):
    descs = [
        "CIE-XYZ-D65 to AP0 (Un-tone-mapped inverse)",
        "ACES AP0 to AP1 matrix",
        "Display encode (gamma) from linear AP1",
    ]
    if has_domain_range:
        descs.append("Normalize display-shaper domain to [0,1]")
    descs += [
        "3D LUT - display-encoded AP1 to ACEScct(scene)",
        "ACEScct log to linear (decode LUT output)",
        "ACES AP1 to AP0 matrix",
    ]
    return descs
...
def generate_clf_files_display_shaper(
    ...
    Uses gamma 2.2 (SDR) or per-VT gamma (HDR) at AP1 primaries as the LUT domain
    ...
    for vt_name, builtin_style in vt_list:
        ...
        has_range = domain_min is not None
        postprocess_clf(
            fwd_path,
            name=f"{vt_name} - Forward",
            clf_id=f"urn:aswf:ocio:transformId:{base}:fwd",
            input_desc='ACES2065-1',
            output_desc='CIE-XYZ-D65',
            op_descriptions=_fwd_op_descriptions_dispshaper(has_range),
        )
        postprocess_clf(
            inv_path,
            name=f"{vt_name} - Inverse",
            ...
            input_desc='CIE-XYZ-D65',
            output_desc='ACES2065-1',
            op_descriptions=_inv_op_descriptions_dispshaper(has_range),
        )

        fwd_size = os.path.getsize(fwd_path)
        inv_size = os.path.getsize(inv_path)
        domain_str = f"{domain_min:.4f} .. {domain_max:.4f}" if has_range else "full [0,1]"
        print(f"    {vt_name}  [shaper: {shaper_label}, domain: {domain_str}]")
        print(f"      Forward: {fwd_clf} ({fwd_size / 1024:.0f} KB, {n_fwd} ops)")
        print(f"      Inverse: {inv_clf} ({inv_size / 1024:.0f} KB, {n_inv} ops)")
...
def migrate_interchange_to_description(text):
    ...
    interchange_pat = re.compile(
        ...
        r'(?P=indent)(?P<sub_indent>[ \t]+)amf_transform_ids:\s*\|\s*\n',
        re.MULTILINE,
    )
    for m in reversed(list(interchange_pat.finditer(text))):
        ic_indent = m.group('indent')
        sub_indent = m.group('sub_indent')
        min_urn_indent = len(ic_indent) + len(sub_indent) + 2
        ...
        while pos < len(text):
            ...
            if leading >= min_urn_indent:
                urns.append(stripped.strip())
                pos = nl + 1
            else:
                break
        ic_block_end = pos
        ...
        if desc_abs_start is not None:
            ...
            text = text[:m.start()] + replacement + text[ic_block_end:]
...
def downgrade_v23_to_v21(config_path, output_path=None):
    ...
    for builtin_style, lut_name in CSC_BUILTINS_V22.items():
        ...
        if matrix_values:
            ...
            def _csc_replacer(m, _lut=lut_rel, _mat=mat_str):
                start = m.start()
                preceding = text[:start]
                last_nl = preceding.rfind('\n')
                line_before = preceding[last_nl + 1:] if last_nl >= 0 else preceding
                is_list_item = '- ' in line_before
                if is_list_item:
                    indent = line_before[:line_before.index('- ')]
                    ci = indent + '    '
                    ii = ci + '  '
                    return (
                        f'!<GroupTransform>\n'
                        f'{ci}children:\n'
                        f'{ii}- !<FileTransform> {{src: {_lut}, interpolation: linear}}\n'
                        f'{ii}- !<MatrixTransform> {{matrix: [{_mat}]}}'
                    )
                return (
                    f'!<GroupTransform>\n'
                    f'      children:\n'
                    f'        - !<FileTransform> {{src: {_lut}, interpolation: linear}}\n'
                    f'        - !<MatrixTransform> {{matrix: [{_mat}]}}'
                )

            text = re.sub(pattern, _csc_replacer, text)
        else:
            replacement = f'!<FileTransform> {{src: {lut_rel}, interpolation: linear}}'
            text = re.sub(pattern, replacement, text)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Inverse encoding: display-shaper (default, gamma 2.2 for SDR / per-VT gamma for HDR "
    ...
    if args.inv_encoding == "acescct-domain":
        ...
        print("  Display-shaper: gamma 2.2 (SDR) / per-VT gamma (HDR) at AP1 primaries")
    ...
    inv_labels = {
        ...
        "display-shaper": "display-shaper (gamma 2.2 SDR / per-VT gamma HDR)",
        "acescc": "ACEScc",
        "acescct": "ACEScct (LogCamera shaper)",
